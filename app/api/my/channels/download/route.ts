  ...
  try {
    ...
    while (true) {
      ...
      for (const item of items) {
        allItems.push({ id: Number(item.id), name: String(item.name || ""), priority: parsePriority(item.channel_json), listQuota: Number(item.used_quota) || 0, created_at: String(item.created_at || "") });
      }
