...
/** 站点 id → 名称（与 supplier PUBLISH_SITES 一致） */
...
/** 从 status-batch 响应解析当前页渠道的状态（纯函数，只提取每站 status 与 key 统计摘要） */
function parseStatusPage(data: unknown): Map<number, { sites: { site_id: number; status: number }[]; multiKeySize: number; aliveCount: number; deadCount: number }> {
  ...
  for (const [idStr, entry] of Object.entries(map)) {
    ...
    for (const s of sites) {
      ...
      if (!multiKeySize && typeof info.multi_key_size === "number") {
        multiKeySize = info.multi_key_size;
      }
      const list = info.multi_key_status_list;
      let deadOnSite: number | null = null;
      if (list && typeof list === "object") {
        const statuses = Object.values(list as Record<string, unknown>).map(x => Number(x));
      ...
      if (deadOnSite !== null) {
        ...
        if (st === 1) {
          if (minDeadOpen === null) minDeadOpen = deadOnSite; else minDeadOpen = Math.min(minDeadOpen, deadOnSite);
        }
      }
    }
...
export async function GET(req: NextRequest) {
  try {
    ...
    // 2. 并行拉 used-quota + status-batch（只拉当前页的渠道）
    let usageMap = new Map<number, number>(); // channelId → used_quota
